    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($serviceTypes as $serviceType)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $serviceType->id }}
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-900">
                                            {{ $serviceType->description ? Str::limit($serviceType->description, 50) : 'N/A' }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        {{ $serviceType->budgets->count() }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-2">
                                            ...
                                            <form method="POST" action="{{ route('service-types.destroy', $serviceType) }}"
                                                class="inline"
                                                onsubmit="return confirm('Tem certeza que deseja excluir este tipo de serviço?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                ...
                @if($serviceTypes instanceof \Illuminate\Pagination\LengthAwarePaginator)
                    <div class="px-6 py-4 bg-white border-t border-gray-200">
                        {{ $serviceTypes->appends(request()->query())->links() }}
                    </div>
                @endif
            </div>
        </div>
