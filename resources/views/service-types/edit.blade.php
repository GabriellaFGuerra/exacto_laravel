    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Editar Tipo de Serviço') }} - {{ $serviceType->name }}
            </h2>
                    ...
                    <form method="POST" action="{{ route('service-types.update', $serviceType) }}">
                        ...
                        <div class="grid grid-cols-1 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <select name="status" id="status"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    <option value="1" {{ old('status', $serviceType->status) == 1 ? 'selected' : '' }}>
                                        Ativo</option>
                                    <option value="0" {{ old('status', $serviceType->status) == 0 ? 'selected' : '' }}>
                                        Inativo</option>
                                </select>
                                @error('status')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="description" id="description" rows="4"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                placeholder="Descrição detalhada do tipo de serviço...">{{ old('description', $serviceType->description) }}</textarea>
                            @error('description')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
