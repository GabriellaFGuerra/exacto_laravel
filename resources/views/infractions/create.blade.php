                    ...
                    <form method="POST" action="{{ route('infractions.store') }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <select name="type" id="type"
                                    ...
                                    <option value="velocidade" {{ old('type') == 'velocidade' ? 'selected' : '' }}>Excesso
                                        de Velocidade</option>
                                    <option value="semaforo" {{ old('type') == 'semaforo' ? 'selected' : '' }}>Avanço de
                                        Semáforo</option>
                                    <option value="estacionamento" {{ old('type') == 'estacionamento' ? 'selected' : '' }}>Estacionamento Irregular</option>
                                    <option value="documento" {{ old('type') == 'documento' ? 'selected' : '' }}>Documento
                                        Irregular</option>
                            ...
                            <div>
                                ...
                                <input type="text" name="license_plate" id="license_plate"
                                    value="{{ old('license_plate') }}" placeholder="ABC-1234"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                    required>
                                @error('license_plate')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <select name="status" id="status"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    <option value="active" {{ old('status') == 'active' ? 'selected' : '' }}>Ativa
                                    </option>
                                    <option value="paid" {{ old('status') == 'paid' ? 'selected' : '' }}>Paga</option>
                                    <option value="appealed" {{ old('status') == 'appealed' ? 'selected' : '' }}>Recorrida
                                    </option>
                            ...
                            <div>
                                <label for="auto_number" class="block text-sm font-medium text-gray-700">Número do
                                    Auto</label>
                                <input type="text" name="auto_number" id="auto_number" value="{{ old('auto_number') }}"
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="observation" id="observation" rows="4"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                placeholder="Observações adicionais sobre a infração...">{{ old('observation') }}</textarea>
                            @error('observation')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
